...
const bookTicket = async (req, res) => {
  ...
  try {
    ...
    if (event.availableTickets > 0) {
      ...
      const booking = new Booking({ user, event: eventId, status: 'confirmed' });
      ...
      await session.commitTransaction();
      res.status(200).json({ message: 'Event sold out, added to waiting list', waitingList });
    }
  } catch (error) {
...
const cancelBooking = async (req, res) => {
    ...
    try {
      ...
      const booking = await Booking.findOneAndDelete({ user, event: eventId }).session(session);
      if (!booking) {
      ...
      const nextInLine = await WaitingList.findOneAndDelete({ event: eventId }).session(session);
      if (nextInLine) {
        // Create a new booking for the next user in line
        const newBooking = new Booking({ user: nextInLine.user, event: eventId, status: 'confirmed' });
  ...
  const viewEventStatus = async (req, res) => {
    try {
      ...
      const waitingListCount = await WaitingList.countDocuments({ event: eventId });
...
module.exports = { initializeEvent, bookTicket, cancelBooking, viewEventStatus };
